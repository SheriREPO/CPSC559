...
# ─────────────────────────────────────────────────────────
# Global registry — every Server instance registers here
# The dashboard HTTP API reads from this dict
# ─────────────────────────────────────────────────────────
_registry: dict[int, "Server"] = {}

# Global task log — all completed tasks stored here for the dashboard
_task_log: list[dict] = []
...
class Server:
    ...
    def __init__(self, server_id, log=None, on_status_change=None):
        self.id                   = server_id
        self.log                  = log or print
        self.on_status_change     = on_status_change

        # ── Network layer (pure TCP sockets) ──
        self.net                  = Network(server_id, self.log)
    ...
    async def handle_broadcast(self, msg):
        ...
        if msg_type == "ELECTION":
            sender = msg["from"]
            if sender < self.id:
                self.log(f"[Server {self.id}] ELECTION from {sender} → sending OK")
            ...
            if self.role == "leader":
                ...
                # Store in global task log for dashboard
                _task_log.append({
                    "task_id":  task_id,
                    "task":     msg.get("task", ""),
    ...
    async def handle_task_submission(self, msg):
        ...
        if worker_id is None:
            self.log(f"[Leader {self.id}] No workers available for task {task_id}")
            return
    ...
    async def handle_worker_task(self, msg):
        ...
        # Call FastAPI
        result = await self.execute_task(category, task_name, msg)
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    ...
                    if resp.status == 200:
                        ...
                        self.log(f"[Worker {self.id}] API ← {data}")
                        return str(data)
    ...
    async def _monitor_workers(self):
        while self.alive and self.role == "leader":
            ...
            for wid in list(self.worker_last_hb.keys()):
                if now - self.worker_last_hb.get(wid, 0) > WORKER_HB_TIMEOUT:
    ...
    def snapshot(self) -> dict:
        """Returns the current state of this node as a dict for the dashboard."""
        return {
            "id":              self.id,
            "role":            self.role,
            ...
            "tasks_done":      self.task_counter,
            "election_active": self.election_in_progress,
            "workers_known":   list(self.worker_last_hb.keys()),
        }
